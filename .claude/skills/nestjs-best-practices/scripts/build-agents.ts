...
/**
 * Build script for generating AGENTS.md from individual rules files
 *
 * Usage: npx ts-node scripts/build-agents.ts
 *
 * This script:
 * 1. Reads all rules files from the rules/ directory
...
function generateTableOfContents(rulesByCategory: Map<string, Rule[]>): string {
  ...
  for (const cat of CATEGORIES) {
    ...
    for (let i = 0; i < rules.length; i++) {
      const rule = rules[i];
      // Rule anchor format: #11-rules-title
...
`;
  ...
  for (const cat of CATEGORIES) {
    ...
    for (let i = 0; i < categoryRules.length; i++) {
      ...
      // Add rules header with number (anchor will be auto-generated as #11-title)
      doc += `### ${ruleNumber} ${rule.frontmatter.title}\n\n`;
      doc += `**Impact: ${rule.frontmatter.impact}** — ${rule.frontmatter.impactDescription}\n\n`;

      // Add rules content (skip the first header since we already added it)
